...
{#if focused}
  <div class="fixed inset-0 md:bg-black/50 bg-white"></div>
{/if}

<form action="/search" method="GET" class="relative" onsubmit={handleSubmit}>
  <div class="relative bg-white overflow-hidden rounded-t-[19px] border border-neutral-300" class:rounded-[19px]={hideResults} >
    <input
      type="text"
      placeholder="Search..."
      bind:value={query}
      bind:focused={focused}
      class="w-full px-4 py-1.5 pr-16 focus:outline-none appearance-none border-none"
    />
  ...
  <ul class="absolute left-0 right-0 overflow-hidden bg-white border border-t-0 border-neutral-300 rounded-b-[19px] shadow-lg" class:hidden={hideResults}>
